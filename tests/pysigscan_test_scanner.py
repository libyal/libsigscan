...
def main():
    ...
    random_data = b"\x01\xfa\xe0\xbe\x99\x8e\xdb\x70\xea\xcc\x6b\xae\x2f\xf5\xa2\xe4"
    ...
    if not pysigscan_test_scan_buffer(scanner, lnk_pattern, expected_scan_results):
        return 1
    ...
    if not pysigscan_test_scan_buffer(scanner, regf_pattern, expected_scan_results):
        return 1

    expected_scan_results = []
    if not pysigscan_test_scan_buffer(scanner, random_data, expected_scan_results):
        return 1
