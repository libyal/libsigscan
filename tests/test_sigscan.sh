...
{ 
	...
	${TEST_RUNNER} ${TMPDIR} ${TEST_EXECUTABLE} -c ../etc/sigscan.conf ${OPTIONS} ${INPUT_FILE} | sed '1,2d' > ${TEST_RESULTS};
...
SCAN_TOOL="../${TEST_PREFIX}tools/${TEST_PREFIX}";
...
if ! test -x "${SCAN_TOOL}";
then
	echo "Missing executable: ${SCAN_TOOL}";
...
run_tests "${TEST_PREFIX}" "${TEST_PREFIX}" "${SCAN_TOOL}";
