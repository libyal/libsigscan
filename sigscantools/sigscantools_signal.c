...
void (*sigscantools_signal_signal_handler)( sigscantools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI sigscantools_signal_handler(
             sigscantools_signal_t signal )
{
	static char *function = "sigscantools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( sigscantools_signal_signal_handler != NULL )
			{
				sigscantools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void sigscantools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int sigscantools_signal_attach(
     void (*signal_handler)( sigscantools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "sigscantools_signal_attach";
	...
	sigscantools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) sigscantools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	sigscantools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int sigscantools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "sigscantools_signal_detach";
	...
	sigscantools_signal_signal_handler = NULL;
